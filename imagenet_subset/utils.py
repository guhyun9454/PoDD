...
def create_simple_buffer(model, args, num_classes):
    """간단한 테스트용 buffer를 생성합니다"""
    print("Creating simple buffer for testing...")
    net = get_network(model, args.channel, num_classes, args.im_size).to(args.device)
    buffer = []
    
    # 매우 간단한 buffer 생성 (짧은 trajectory)
    trajectory_length = args.max_start_epoch + args.expert_epochs + 1
    trajectory = []
    
    # 초기 모델 파라미터
    init_params = [p.clone().detach() for p in net.parameters()]
    
    # 간단한 trajectory 생성
    for i in range(trajectory_length):
        # 약간씩 변화하는 파라미터 생성
        params = [p.clone().detach() + torch.randn_like(p) * 0.01 * i for p in init_params]
        trajectory.append(params)
    
    buffer.append(trajectory)
    
    # develop 모드에서는 buffer 크기를 작게 유지
    simple_buffer_size = 5
    for _ in range(simple_buffer_size - 1):
        # 약간의 변화를 주어 여러 trajectory 생성
        new_trajectory = []
        for i in range(trajectory_length):
            params = [p.clone().detach() + torch.randn_like(p) * 0.005 for p in trajectory[i]]
            new_trajectory.append(params)
        buffer.append(new_trajectory)
    
    print(f"Created simple buffer with {len(buffer)} trajectories of length {trajectory_length}")
    return buffer
