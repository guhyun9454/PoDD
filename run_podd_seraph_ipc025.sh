#!/bin/bash
# PoDD on Seraph/ariel, ImageNet-Nette 128^2 at IPC 0.25.
#
# Identical to run_podd_seraph.sh (the completed IPC 1.0 run, jobs 391739/397617/404644)
# except the poster budget: 640x256 * sqrt(0.25) -> 320x128 = 40,960 px = exactly
# 0.25 * 10 * 128^2. The poster is now exactly one class-area tall, so a 4-row patch grid
# would collapse to 4 duplicate rows (linspace(0, 0, 4)); the grid is 20x1 instead, keeping
# total_patch_num = 20 identical to the IPC 1.0/0.5 protocol. The two class rows fully
# overlap and fade-blend vertically (verified safe in combine_images_with_fade), and the
# exact 2.5:1 aspect keeps PoDDL.init_label_array on the correct (2,5) label grid.
#
...
python -u main.py \
  ...
  --poster_class_num_x 5 --poster_class_num_y 2 --poster_width 320 --poster_height 128 \
  --class_area_width 128 --class_area_height 128 --patch_num_x 20 --patch_num_y 1 \
